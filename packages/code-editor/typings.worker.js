...
const getRequireStatements = (title, code) => {
  ...
  const sourceFile = workerSelf.ts.createSourceFile(
    title,
    code,
    workerSelf.ts.ScriptTarget.Latest,
    true,
    workerSelf.ts.ScriptKind.TS
  )

  workerSelf.ts.forEachChild(sourceFile, (node) => {
    switch (node.kind) {
      case workerSelf.ts.SyntaxKind.ImportDeclaration: {
        requires.push(node.moduleSpecifier.text)
        break
      }
      case workerSelf.ts.SyntaxKind.ExportDeclaration: {
